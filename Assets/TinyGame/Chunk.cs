    ...
    public class Chunk
    {
        ...
        public Chunk(CastleGrid origin)
        {
            ...
            for (var x = 1; x < ChunkSize; x++)
            {
                for (var y = 1; y < ChunkSize; y++)
                {
                    ...
                    if(HasTerrain(x+1,y) == h)
                        continue;
                    if(HasTerrain(x,y+1) == h)
                        continue;
                    if(HasTerrain(x-1,y) == h)
                        continue;
                    if(HasTerrain(x,y-1) == h)
                        continue;
                    if(HasTerrain(x+1,y+1) == h)
                        continue;
                    if(HasTerrain(x-1,y-1) == h)
                        continue;
                    if(HasTerrain(x+1,y-1) == h)
            ...
            for (var x = 0; x < ChunkSize; x++)
            {
                for (var y = 0; y < ChunkSize; y++)
                {
                    ...
                    if (hasTerrain[x * NodeSize + y])
                    {
                        voxelBits[x * ChunkSize + y] += 1;
                    }
                    if (hasTerrain[(x + 1) * NodeSize + y])
                    {
                        voxelBits[x * ChunkSize + y] += 2;
                    }
                    if (hasTerrain[x * NodeSize + y + 1])
                    {
                        voxelBits[x * ChunkSize + y] += 4;
                    }
                    if (hasTerrain[(x + 1) * NodeSize + y + 1])
                    {
                        voxelBits[x * ChunkSize + y] += 8;
                    }
                }
            }
            MakeObjects();
        ...
        public void MakeTrees()
        {
            ...
            for (var x = 0; x < ChunkSize; x++)
            {
                for (var y = 0; y < ChunkSize; y++)
                {
                    if (voxelBits[x * ChunkSize + y] switch
                        {
                            7 or 11 or 13 or 14 or 15 => false,
                            _ => true
                        })
                    {
                        continue;
        ...
        public static CastleGrid ChunkPosition(int x, int y) => new(x / ChunkSize, y / ChunkSize);
